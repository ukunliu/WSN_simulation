function [meta, tx_set, rx_set, rays] = generate_test(coord, n_x, n_y, n_rx)
...
x = linspace(lon_s, lon_e, n_x);
y = linspace(lat_s, lat_e, n_y);
[X, Y] = meshgrid(x, y);

[X_rx, Y_rx] = meshgrid( X(1, n_x/n_rx: n_x/n_rx * 2: n_x), Y(n_y/n_rx: n_y/n_rx*2: n_y, 1));
...
ele_matrix = reshape(ele, n_x, n_y);
...
%               tmp.ChannelFiltering = 0;
              ...
              resp_cell{i, j} = [cast(delay, 'double'); gain];
        end
    end
end
